    ...
    def forward(self, trg_seq, num_negs, user):
        ...
        if self.state == 'training':
            for check_in in trg_seq:
                trg_loc = check_in[1]
                nearby_locs = self.loc_query_sys.retrieve(trg_loc, k=self.n_nearest)
                samples = []
...
class UniformSampler(nn.Module):
    ...
    def forward(self, trg, num_neg, user):
        # random pick num_neg loc and generate a tensor of (len(trg), num_neg)
        return torch.randint(low=1, high=self.n_loc, size=[len(trg), num_neg])